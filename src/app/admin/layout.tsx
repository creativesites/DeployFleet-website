...
export const metadata: Metadata = {
  title: { template: "%s — DeployFleet Admin", default: "Admin" },
...
/**
 * Two independent gates, both required, checked server-side once here for
 * every /admin/* route — previously only ever ran inside the single
 * /admin/page.tsx, which worked when the dashboard was one route with
 * client-side tab state, but a layout is the correct place for this now
 * that each former tab is a real route (spec-driven redesign: sidebar
 * navigation, not tabs, so the URL actually reflects which page you're
 * on — bookmarkable, shareable, back-button-correct).
 *
 * 1. Clerk auth — proves *who* the visitor is. src/proxy.ts already
 *    redirects a signed-out visitor to sign in before this layout ever
 ...
 *    automatically trusted with lead/visitor data; see adminAccess.ts.
 */
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  if (!isAdmin) {
    return (
      <NotConfigured>
        You&apos;re signed in, but this account isn&apos;t on the <code>ADMIN_ALLOWED_EMAILS</code> allowlist. Ask
        whoever manages this deployment to add your email if you should have access.
      </NotConfigured>
    );
  }
