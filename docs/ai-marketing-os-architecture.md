...
- **Superseded: "No call automation anywhere, ever — calls and WhatsApp stay entirely human-mediated."** That line was correct as originally scoped; Winston has since asked for controlled WhatsApp automation specifically (calls remain out of scope everywhere). See **`docs/whatsapp-intelligence-architecture.md`** — Phase 4 of this plan, a full architecture document in its own right, grounded in an actual audit of Zuri (`creativesites/Personal-Assistant`)'s existing WhatsApp/conversation-intelligence stack. All five of its sub-phases (WA-0 gateway service through WA-4 Orchestrator/Command Center integration) are now built — see that doc's own status and README.md's writeup. The gateway (`whatsapp-service/`) is code-complete and type-verified but not live-deployed or connected to a real WhatsApp account (needs a phone to scan a QR code and hosting-provider credentials, neither available in this dev environment).
