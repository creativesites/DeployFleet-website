...
- **No outbound automation is in scope anywhere in this plan.** The Orchestrator can draft things and create tasks; it never calls, WhatsApps, or emails a prospect itself. Confirmed as the right boundary for a personal-use system by the plan's own "External World stays human-mediated" note in §3, but worth Winston explicitly re-confirming before Phase 2 starts, since the brief's own language ("AI can call/message") gestures toward more automation than this plan commits to.
