...
**Status: planning document, not yet implemented.** Written before any code changes, per the same discipline `docs/ai-marketing-os-architecture.md` was written under — the full shape of the system agreed first. Nothing in this document should be treated as built until a later session's commits say otherwise.
...
## 15. Open questions & risks — flagged, not resolved here

Matching this project's own standing discipline of surfacing what a plan doesn't answer rather than silently picking one:

- **`WhatsAppConversation.state` vs. `Prospect.stage` reconciliation is genuinely unresolved** (§9) — they're related but can legitimately diverge (a conversation can reach `interested` while the prospect's own pipeline stage hasn't advanced, e.g. pending a second stakeholder). Whether the conversation state ever *writes* the prospect stage, or stays purely observational, needs a real decision before WA-2 ships, not an assumption baked in silently.
- **Hosting choice (Railway vs. Fly.io vs. VPS) is a real decision with real cost/ops tradeoffs**, recommended in §13 but not made — needs Winston's own sign-off before WA-0 starts, the same way the base doc left the Odoo-edition and launch-market questions open for a human decision rather than picking one.
- **The dedicated-WhatsApp-number acquisition is an external dependency** (a real business number, a real WhatsApp Business account) — not something this codebase or this document can provision; flagged so it isn't discovered as a blocker mid-WA-0.
- **Whether `checkAvailability()` itself risks WhatsApp's own rate limits or account-health signals is unverified** — Baileys' `onWhatsApp` primitive is a real library capability, but neither Zuri nor DeployFleet has production experience calling it at any volume; WA-1 should start with manual, low-volume verification and watch for any account-health warnings before assuming it's safe to run automatically at scale.
- **This document, like the base doc before it, has zero live-provider/live-WhatsApp verification behind any of it** — every claim about Baileys/Zuri behavior traces to source code actually read (§2.2), not to a live test run, since this dev environment has no WhatsApp account or Baileys session to test against. The base doc's own tool-calling work carried this exact caveat through Phase 2; this phase inherits it for the same reason.
- **No decision yet on whether `WhatsAppMessage`/`WhatsAppConversation` should be their own Firestore collections (as designed in §5.3) or nested under `Prospect`** — the flat-collection design here follows every other phase's own precedent (`Fact`, `Task`, `Decision` are all flat, filtered by `prospectId`), but worth a final look once real message volume is observed, since a very chatty prospect could make `whatsappMessages` the single largest collection in the system.
